...
all(colnames(called)  %in% pd_example$SAMPLE_ID)
colnames(called) <- ss(colnames(called),"_accepted")
all(colnames(called)  %in% pd_example$SAMPLE_ID)
...
```{r}
snpsRNA <- make_snpsRNA(snpsGeno_VCF, called_filter)
print("Matching Genotype snps")
snpsRNA$snpsGeno2[1:5, 1:5]
print("Called RNA snps")
